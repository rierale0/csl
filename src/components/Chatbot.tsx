...
const splitText = (text: string) => {
  ...
  for (let i = 0; i < paragraphs.length; i += 2) {
    groupedParagraphs.push(
      [paragraphs[i], paragraphs[i + 1]].filter(Boolean).join("\n\n")
    );
  }
...
interface ChatbotProps {
  ...
  ctaText?: string[];  // Changed from string to string[]
...
const Chatbot: React.FC<ChatbotProps> = ({
  logoPath = "/images/csl-logo-dark.svg",
  ...
  ctaText = ["✨ Solicitar una cotización", "💡 Consultar servicios"],  // Default array of CTAs
  ctaIcon = <ArrowRightIcon />,
  inputPlaceholder,
  poweredByText,
  brandName = "Pycat",
  darkMode = true,
  fontFamily = "'Poppins', system-ui, 'Open Sans'",
  companyName = "",
  ...
  const styles = {
    ...
    chatWindow: {
      ...
      backgroundColor: darkMode ? "#2D3239" : "#f5f5f7",
      borderRadius: "10px",
      boxShadow: darkMode
  ...
  const sendMessage = async () => {
    if (inputText.trim()) {
      ...
      const isCTA = ctaText.includes(userMessage.text);
      ...
      try {
        ...
        let botResponseSplit: string[] = splitText(botResponseText);

        for (let i = 0; i < botResponseSplit.length; i++) {
          setIsTyping(true);
          await new Promise((resolve) => setTimeout(resolve, 3000));
          const botResponse: ChatMessage = {
            text: botResponseSplit[i],
            isUser: false,
            timestamp: new Date().toISOString()
          };
        ...
        const errorMessage = {
          text: error instanceof DOMException && error.name === 'AbortError' 
            ? "Lo siento, la respuesta está tardando más de lo esperado. Por favor, intenta de nuevo."
          ...
          >
            ...
            {showIntro ? (
              ...
              >
                ...
                {ctaText.map((text, index) => (
                  <button
                    key={index}
                    style={{
                      ...styles.ctaButton,
                      marginBottom: index < ctaText.length - 1 ? "10px" : "20px",
                    }}
                    onClick={async () => {
                      // Create the user message first
                      const newMessage: ChatMessage = {
                        text: text, // Fixed: Use the individual CTA text
                        isUser: true,
                        timestamp: new Date().toISOString()
                      };
                      ...
                      setTimeout(() => {
                        ...
                        fetch(webhookUrl, {
                          ...
                          body: JSON.stringify({
                            message: text, // Fixed: Use the individual CTA text
                            timestamp: new Date().toISOString(),
                            userId: userId,
                            isCTA: true,
                          }),
            ...
            <div
              style={{
                ...
                overflowY: "auto",
                height: "100%",
                paddingRight: "10px", // Added padding to the right for the scrollbar
                scrollbarWidth: "thin",
                scrollbarColor: `${
              ...
              <style>
                {`
                  .custom-scrollbar::-webkit-scrollbar {
                    ...
                    margin-right: 10px; // Increased margin-right for more space
                  ...
                  .custom-scrollbar::-webkit-scrollbar-thumb {
                    ...
                    border-right: 6px solid transparent; // Increased border-right for more space
